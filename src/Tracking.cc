...
namespace ORB_SLAM3
{
        ...
        mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
    {
        ...
        mImGray = cv::Mat(mImHeight, mImWidth, CV_8U);
    ...
    void Tracking::newParameterLoader(Settings* settings) {
        cv::Size size = settings->newImSize();
        mImWidth = size.width;
        mImHeight = size.height;
    ...
    Sophus::SE3f Tracking::GrabFrameMonocular(Frame& frame, const double& timestamp, string filename)
    {
        if (mSensor == System::MONOCULAR)
        {
            if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET || (lastID - initID) < mMaxFrames)
                frame.completeFrame(timestamp, mpIniORBextractor, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth, mImWidth, mImHeight);
            else
                frame.completeFrame(timestamp, mpORBextractorLeft, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth, mImWidth, mImHeight);
            mCurrentFrame = frame;
        }
        /*
        else if(mSensor == System::IMU_MONOCULAR)
        {
            mCurrentFrame = Frame(frame, timestamp, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth, &mLastFrame, *mpImuCalib);
        }*/
    ...
    void Tracking::PreintegrateIMU()
    {
        ...
        if (mlQueueImuData.size() == 0)
        {
            ...
            std::cout << "Not IMU data in mlQueueImuData!!" << endl;
            mCurrentFrame.setIntegrated();
            return;
        }
        ...
        else
        {
            ...
            if (!mbOnlyTracking)
            {
                ...
                if (mState == OK)
                {
                    ...
                    if ((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId < mnLastRelocFrameId + 2)
                    {
                        ...
                        std::cout << "TRACK: Track with respect to the reference KF " << endl;
                        bOK = TrackReferenceKeyFrame();
                    }
                    else
                    {
                        Verbose::PrintMess("TRACK: Track with motion model", Verbose::VERBOSITY_DEBUG);
                        //std::cout << "TRACK: Track with motion model" << endl;
                ...
                else
                {
                    ...
                    if (mState == RECENTLY_LOST)
                    {
                        ...
                        std::cout << "Lost for a short time" << endl;
                        bOK = true;
                    ...
                    else if (mState == LOST)
                    {
                        ...
                        std::cout << "A new map is started..." << endl;
            ...
            else if (mState == OK)
            {
                if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
                {
                    ...
                    std::cout << "Track lost for less than one second..." << endl;
                ...
                (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && pCurrentMap->isImuInitialized())
            {
                ...
                std::cout << "Saving pointer to frame. imu needs reset..." << endl;
            ...
            // Update drawer
            mpFrameDrawer->Update(this);
            if (mCurrentFrame.isSet() && mState == OK)
            ...
            if (mState == LOST)
            {
                ...
                if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
                    if (!pCurrentMap->isImuInitialized())
                    {
                        ...
                        std::cout << "Track lost before IMU initialisation, reseting..." << endl;
                        mpSystem->ResetActiveMap();
                        return;
                    }
    ...
    void Tracking::CreateInitialMapMonocular()
    {
        ...
        // Bundle Adjustment
        Verbose::PrintMess("New Map created with " + to_string(mpAtlas->MapPointsInMap()) + " points", Verbose::VERBOSITY_QUIET);
        std::cout << "New Map created with " + to_string(mpAtlas->MapPointsInMap()) + " points" << endl;
    ...
    void Tracking::CreateMapInAtlas()
    {
        ...
        std::cout << "First frame id in map: " + to_string(mnLastInitFrameId + 1) << endl;
    ...
    bool Tracking::TrackWithMotionModel()
    {
        ...
        if (nmatches < 20)
        {
            ...
            std::cout << "Not enough matches, wider window search!!" << endl;
            fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(), static_cast<MapPoint*>(NULL));

            nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, 2 * th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
            Verbose::PrintMess("Matches with wider search: " + to_string(nmatches), Verbose::VERBOSITY_NORMAL);
            std::cout << "Matches with wider search: " + to_string(nmatches) << endl;
    ...
    bool Tracking::TrackLocalMap()
    {
        ...
        else
        {
            if (mCurrentFrame.mnId <= mnLastRelocFrameId + mnFramesToResetIMU)
            {
                ...
                std::cout << "TLM: PoseOptimization " << endl;
                Optimizer::PoseOptimization(&mCurrentFrame);
            }
            else
            {
                ...
                if (!mbMapUpdated) //  && (mnMatchesInliers>30))
                {
                    ...
                    std::cout << "TLM: PoseInertialOptimizationLastFrame " << endl;
    ...
    void Tracking::CreateNewKeyFrame()
    {
        ...
        else
        {
            ...
            std::cout << "No last KF in KF creation!!" << endl;
        }
    ...
    void Tracking::Reset(bool bLocMap)
    {
        ...
        if (!bLocMap)
        {
            ...
            std::cout << "Reseting Local Mapper..." << std::endl;
        ...
        std::cout << "Reseting Loop Closing..." << std::endl;
        ...
        std::cout << "   End reseting! " << std::endl;
    }

    void Tracking::ResetActiveMap(bool bLocMap)
    {
        Verbose::PrintMess("Active map Reseting", Verbose::VERBOSITY_NORMAL);
        std::cout << "Active map Reseting" << std::endl;
        if (mpViewer)
        {
            mpViewer->RequestStop();
            while (!mpViewer->isStopped())
